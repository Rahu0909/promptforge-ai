...
public class ConversationController {
    ...
    public ResponseEntity<ApiResponse<ConversationResponse>> create(@Valid @RequestBody CreateConversationRequest request) {
        return ResponseEntity.ok(ApiResponse.success(conversationService.create(request), "Conversation created successfully."));
    }

    @GetMapping("/{conversationId}")
    @Operation(summary = "Get Conversation")
    public ResponseEntity<ApiResponse<ConversationResponse>> get(@PathVariable UUID conversationId) {
        return ResponseEntity.ok(ApiResponse.success(conversationService.get(conversationId), "Conversation fetched successfully."));
    }

    @GetMapping("/project/{projectId}")
    @Operation(summary = "Search Project Conversations")
    public ResponseEntity<ApiResponse<PageResponse<ConversationResponse>>> list(
            @PathVariable UUID projectId,
            @RequestParam(required = false) String search,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        return ResponseEntity.ok(ApiResponse.success(conversationService.getProjectConversations(projectId, search, page, size), "Conversations fetched successfully."));
    }

    @PatchMapping("/{conversationId}")
    @Operation(summary = "Rename Conversation")
    public ResponseEntity<ApiResponse<ConversationResponse>> rename(@PathVariable UUID conversationId, @Valid @RequestBody RenameConversationRequest request) {
        return ResponseEntity.ok(ApiResponse.success(conversationService.rename(conversationId, request), "Conversation renamed successfully."));
    }
    ...
    @Operation(summary = "Conversation Statistics")
    public ResponseEntity<ApiResponse<ConversationStatsResponse>> stats(@PathVariable UUID conversationId) {
        return ResponseEntity.ok(ApiResponse.success(conversationService.getStats(conversationId), "Conversation statistics fetched successfully."));
    }
}