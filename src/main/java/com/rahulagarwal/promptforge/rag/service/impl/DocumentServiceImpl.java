...
public class DocumentServiceImpl implements DocumentService {
    ...
    public UploadDocumentResponse upload(UUID projectId, MultipartFile file) {
        ...
        try {
            KnowledgeDocument document = KnowledgeDocument.builder().project(project).documentName(extractDocumentName(file.getOriginalFilename())).originalFileName(file.getOriginalFilename()).documentType(documentType).status(DocumentStatus.UPLOADING).storagePath(storedFile.storagePath()).totalChunks(0).build();
            knowledgeDocumentRepository.save(document);
            return UploadDocumentResponse.builder().documentId(document.getId()).documentName(document.getDocumentName()).status(document.getStatus().name()).build();
        } catch (Exception ex) {
    ...
    public void delete(UUID documentId) {
        KnowledgeDocument document = knowledgeDocumentRepository.findByIdAndDeletedFalse(documentId)
                .orElseThrow(() -> new ResourceNotFoundException("Document not found.", ErrorCode.RESOURCE_NOT_FOUND));
